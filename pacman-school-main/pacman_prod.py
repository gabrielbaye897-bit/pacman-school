...
class Ghost :
    def __init__ (self, color, x, y) :
        ...
        self.pos_x = x
        self.pos_y = y
        self.img = ImageTk.PhotoImage(Image.open(color + "_ghost.png").resize((200,200)))
        area.create_image(x,y,image = self.img, tag="ghost_"+color)
    def bouger (self) :
        dir = randint(1,4)
        if dir == 1 :
            area.move("ghost_"+self.color, 100, 0)
            area.update()
        if dir == 2 :
            area.move("ghost_"+self.color, 0, 100)
            area.update()
        if dir == 3 :
            area.move("ghost_"+self.color, -100, 0)
            area.update()
...
ghosts = []
ghosts.append(Ghost("blue", 500, 0))
ghosts.append(Ghost("yellow", 0, 500))
ghosts.append(Ghost("red", 500, 500))
ghosts.append(Ghost("pink", 250, 250))
